...
class NanoVectorDBStorage(BaseVectorStorage):
    ...
    async def find_similar_entity(self, entity_name: str, entity_description: str, similarity_threshold: float = 0.9) -> Union[str, None]:
        """
        Find existing similar entity based on description embedding similarity.
        ...
        logger.debug(f"Querying similar entities for {entity_name} with description: {entity_description} and threshold: {similarity_threshold}")
...
class NetworkXStorage(BaseGraphStorage):
    ...
    def __post_init__(self):
        self._graphml_xml_file = os.path.join(
            self.global_config["working_dir"], f"event_graph_{self.namespace}.graphml"
        )
        preloaded_graph = NetworkXStorage.load_nx_graph(self._graphml_xml_file)
        if preloaded_graph is not None:
            logger.info(
                f"Loaded event graph from {self._graphml_xml_file} with {preloaded_graph.number_of_nodes()} events, {preloaded_graph.number_of_edges()} relationships"
            )
        self._graph = preloaded_graph or nx.DiGraph()
        self._node_embed_algorithms = {
            "event2vec": self._node2vec_embed,
        }
    ...
    async def find_similar_entity(self, entity_name: str, entity_description: str, entities_vdb: BaseVectorStorage, similarity_threshold: float = 0.9) -> Union[str, None]:
        """
