...
from .cache_on_disk import persistent_cache
...
from .openai_cost_calc import log_openai_usage
...
async def openai_complete_if_cache(
    ...
    **kwargs,
    ...
    response = await _make_openai_request(openai_async_client, model, messages, **kwargs)

    # logger.debug(f"===== Query Output from LLM =====\nResponse: {response}")
...
@retry(stop=stop_after_attempt(10), wait=wait_exponential(multiplier=1, min=4, max=600))
...
async def openai_embedding(
    ...
    response = await _make_openai_embedding_request(
    openai_async_client,
    model=model,
    texts=texts,
...
@log_openai_usage()
async def _make_openai_embedding_request(client, model, texts, encoding_format="float", dimensions=None):
