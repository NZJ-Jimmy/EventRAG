...
class OracleDB:
    ...
    async def check_tables(self):
        for k, v in TABLES.items():
            try:
                if k.lower() == "eventrag_graph":
                    await self.query(
                        "SELECT id FROM GRAPH_TABLE (eventrag_graph MATCH (a) COLUMNS (a.id)) fetch first row only"
                    )
                else:
...
N_T = {
    "full_docs": "EVENTRAG_DOC_FULL",
    "text_chunks": "EVENTRAG_DOC_CHUNKS",
    "chunks": "EVENTRAG_DOC_CHUNKS",
    "entities": "EVENTRAG_GRAPH_NODES",
    "relationships": "EVENTRAG_GRAPH_EDGES",
}

TABLES = {
    "EVENTRAG_DOC_FULL": {
        "ddl": """CREATE TABLE EVENTRAG_DOC_FULL (
    ...
    "EVENTRAG_DOC_CHUNKS": {
        "ddl": """CREATE TABLE EVENTRAG_DOC_CHUNKS (
                    id varchar(256) PRIMARY KEY,
                    workspace varchar(1024),
    ...
    "EVENTRAG_GRAPH_NODES": {
        "ddl": """CREATE TABLE EVENTRAG_GRAPH_NODES (
    ...
    "EVENTRAG_GRAPH_EDGES": {
        "ddl": """CREATE TABLE EVENTRAG_GRAPH_EDGES (
                    id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    workspace varchar(1024),
    ...
    "EVENTRAG_LLM_CACHE": {
        "ddl": """CREATE TABLE EVENTRAG_LLM_CACHE (
                    id varchar(256) PRIMARY KEY,
                    send clob,
                    return clob,
                    model varchar(1024),
    ...
    "EVENTRAG_GRAPH": {
        "ddl": """CREATE OR REPLACE PROPERTY GRAPH eventrag_graph
                VERTEX TABLES (
                    eventrag_graph_nodes KEY (id)
                        LABEL entity
                        PROPERTIES (id,workspace,name) -- ,entity_type,description,source_chunk_id)
                )
                EDGE TABLES (
                    eventrag_graph_edges KEY (id)
                        SOURCE KEY (source_name) REFERENCES eventrag_graph_nodes(name)
                        DESTINATION KEY (target_name) REFERENCES eventrag_graph_nodes(name)
                        LABEL  has_relation
...
SQL_TEMPLATES = {
    # SQL for KVStorage
    "get_by_id_full_docs": "select ID,NVL(content,'') as content from EVENTRAG_DOC_FULL where workspace=:workspace and ID=:id",
    "get_by_id_text_chunks": "select ID,TOKENS,NVL(content,'') as content,CHUNK_ORDER_INDEX,FULL_DOC_ID from EVENTRAG_DOC_CHUNKS where workspace=:workspace and ID=:id",
    "get_by_ids_full_docs": "select ID,NVL(content,'') as content from EVENTRAG_DOC_FULL where workspace=:workspace and ID in ({ids})",
    "get_by_ids_text_chunks": "select ID,TOKENS,NVL(content,'') as content,CHUNK_ORDER_INDEX,FULL_DOC_ID  from EVENTRAG_DOC_CHUNKS where workspace=:workspace and ID in ({ids})",
    "filter_keys": "select id from {table_name} where workspace=:workspace and id in ({ids})",
    "merge_doc_full": """ MERGE INTO EVENTRAG_DOC_FULL a
                    USING DUAL
                    ON (a.id = :check_id)
                    WHEN NOT MATCHED THEN
                    INSERT(id,content,workspace) values(:id,:content,:workspace)
                    """,
    "merge_chunk": """MERGE INTO EVENTRAG_DOC_CHUNKS a
                    USING DUAL
                    ON (a.id = :check_id)
                    WHEN NOT MATCHED THEN
    ...
    "entities": """SELECT name as entity_name FROM
        ...
        FROM EVENTRAG_GRAPH_NODES WHERE workspace=:workspace)
    ...
    "relationships": """SELECT source_name as src_id, target_name as tgt_id FROM
        ...
        FROM EVENTRAG_GRAPH_EDGES WHERE workspace=:workspace)
        WHERE distance>:better_than_threshold ORDER BY distance ASC FETCH FIRST :top_k ROWS ONLY""",
    "chunks": """SELECT id FROM
        (SELECT id,VECTOR_DISTANCE(content_vector,vector(:embedding_string,{dimension},{dtype}),COSINE) as distance
        FROM EVENTRAG_DOC_CHUNKS WHERE workspace=:workspace)
        WHERE distance>:better_than_threshold ORDER BY distance ASC FETCH FIRST :top_k ROWS ONLY""",
    # SQL for GraphStorage
    "has_node": """SELECT * FROM GRAPH_TABLE (eventrag_graph
        MATCH (a)
        WHERE a.workspace=:workspace AND a.name=:node_id
        COLUMNS (a.name))""",
    "has_edge": """SELECT * FROM GRAPH_TABLE (eventrag_graph
        MATCH (a) -[e]-> (b)
    ...
    "node_degree": """SELECT count(1) as degree FROM GRAPH_TABLE (eventrag_graph
        MATCH (a)-[e]->(b)
    ...
    "get_node": """SELECT t1.name,t2.entity_type,t2.source_chunk_id as source_id,NVL(t2.description,'') AS description
        FROM GRAPH_TABLE (eventrag_graph
        MATCH (a)
        WHERE a.workspace=:workspace AND a.name=:node_id
        COLUMNS (a.name)
        ) t1 JOIN EVENTRAG_GRAPH_NODES t2 on t1.name=t2.name
    ...
    "get_edge": """SELECT t1.source_id,t2.weight,t2.source_chunk_id as source_id,t2.keywords,
        ...
        ) t1 JOIN EVENTRAG_GRAPH_EDGES t2 on t1.id=t2.id""",
    ...
    "merge_node": """MERGE INTO EVENTRAG_GRAPH_NODES a
                    USING DUAL
    ...
    "merge_edge": """MERGE INTO EVENTRAG_GRAPH_EDGES a
                    USING DUAL
    ...
    "get_all_nodes": """WITH t0 AS (
                        ...
                        FROM t1 LEFT JOIN eventrag_doc_chunks t2 ON t1.source_chunk_id = t2.id
                        GROUP BY t1.id
                    )
    ...
    "get_all_edges": """SELECT t1.id,t1.keywords as label,t1.keywords, t1.source_name as source, t1.target_name as target,
                ...
                FROM EVENTRAG_GRAPH_EDGES t1
                LEFT JOIN EVENTRAG_DOC_CHUNKS t2 on t1.source_chunk_id=t2.id
    ...
    "get_statistics": """select  count(distinct CASE WHEN type='node' THEN id END) as nodes_count,
                ...
                FROM (
                select 'node' as type, id FROM GRAPH_TABLE (eventrag_graph
                    MATCH (a) WHERE a.workspace=:workspace columns(a.name as id))
                UNION
                select 'edge' as type, TO_CHAR(id) id FROM GRAPH_TABLE (eventrag_graph
