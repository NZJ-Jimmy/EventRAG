...
from eventrag.base import BaseVectorStorage, BaseKVStorage
...
TABLES = {
    "EVENTRAG_DOC_FULL": {
        "ddl": """
        CREATE TABLE EVENTRAG_DOC_FULL (
    ...
    "EVENTRAG_DOC_CHUNKS": {
        "ddl": """
        CREATE TABLE EVENTRAG_DOC_CHUNKS (
            `id` BIGINT PRIMARY KEY AUTO_RANDOM,
    ...
    "EVENTRAG_GRAPH_NODES": {
        "ddl": """
        CREATE TABLE EVENTRAG_GRAPH_NODES (
            `id` BIGINT PRIMARY KEY AUTO_RANDOM,
    ...
    "EVENTRAG_GRAPH_EDGES": {
        "ddl": """
        CREATE TABLE EVENTRAG_GRAPH_EDGES (
            `id` BIGINT PRIMARY KEY AUTO_RANDOM,
    ...
    "EVENTRAG_LLM_CACHE": {
        "ddl": """
        CREATE TABLE EVENTRAG_LLM_CACHE (
...
SQL_TEMPLATES = {
    # SQL for KVStorage
    "get_by_id_full_docs": "SELECT doc_id as id, IFNULL(content, '') AS content FROM EVENTRAG_DOC_FULL WHERE doc_id = :id AND workspace = :workspace",
    "get_by_id_text_chunks": "SELECT chunk_id as id, tokens, IFNULL(content, '') AS content, chunk_order_index, full_doc_id FROM EVENTRAG_DOC_CHUNKS WHERE chunk_id = :id AND workspace = :workspace",
    "get_by_ids_full_docs": "SELECT doc_id as id, IFNULL(content, '') AS content FROM EVENTRAG_DOC_FULL WHERE doc_id IN ({ids}) AND workspace = :workspace",
    "get_by_ids_text_chunks": "SELECT chunk_id as id, tokens, IFNULL(content, '') AS content, chunk_order_index, full_doc_id FROM EVENTRAG_DOC_CHUNKS WHERE chunk_id IN ({ids}) AND workspace = :workspace",
    ...
    "upsert_doc_full": """
        INSERT INTO EVENTRAG_DOC_FULL (doc_id, content, workspace)
        ...
        """,
    "upsert_chunk": """
        INSERT INTO EVENTRAG_DOC_CHUNKS(chunk_id, content, tokens, chunk_order_index, full_doc_id, content_vector, workspace)
    ...
    "entities": """SELECT n.name as entity_name FROM
        ...
        FROM EVENTRAG_GRAPH_NODES WHERE workspace = :workspace) n
    ...
    "relationships": """SELECT e.source_name as src_id, e.target_name as tgt_id FROM
        ...
        FROM EVENTRAG_GRAPH_EDGES WHERE workspace = :workspace) e
    ...
    "chunks": """SELECT c.id FROM
        ...
        FROM EVENTRAG_DOC_CHUNKS WHERE workspace = :workspace) c
        WHERE c.distance>:better_than_threshold ORDER BY c.distance DESC LIMIT :top_k""",
    "upsert_entity": """
        INSERT INTO EVENTRAG_GRAPH_NODES(entity_id, name, content, content_vector, workspace)
        ...
        """,
    "upsert_relationship": """
        INSERT INTO EVENTRAG_GRAPH_EDGES(relation_id, source_name, target_name, content, content_vector, workspace)
